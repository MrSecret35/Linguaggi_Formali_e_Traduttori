...
public class Translator {
    ...
    public void statp(int lnext_stat) {
        ...
        switch(look.tag) {
            ...
            case Tag.COND:
                ...
                bexpr(giusta);
                /*sbagliata*/
                code.emit(OpCode.GOto,sbagliata);
                /*giusta*/
                code.emitLabel(giusta);
                stat(lnext_stat);
            ...
            case Tag.PRINT:
                ...
                */
                ...
                temp_count=count-x;
                for (int i=x; i>0;i--, temp_count++) {
                    code.emit(OpCode.iload,temp_count);
                }
                temp_count=count;
                for (int i=x; i>0;i--, temp_count--) {
                    code.emit(OpCode.invokestatic,1); 
    ...
    private void bexprp(int lnext_bexpr){
        ...
        switch(look.tag){
            ...
            case Tag.AND:
                match(Tag.AND);
                int true_and=code.newLabel();
                int sbagliata=code.newLabel();

                bexpr(true_and);
                code.emit(OpCode.GOto,sbagliata);

                code.emitLabel(true_and);
                bexpr(lnext_bexprp);
            ...
            case '!':
                int true_not=code.newLabel();
                match('!');

                bexpr(true_not);
                code.emit(OpCode.GOto,lnext_bexprp);
                code.emitLabel(true_not);
